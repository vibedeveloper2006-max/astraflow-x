...
/**
 * Global center coordinates for Sawai Mansingh Stadium.
 */
...
/**
 * LiveMap Component
 * Provides a real-time interactive visualization of the stadium zones.
 * Enhanced with accessibility features and Google Maps integration.
 */
export function LiveMap() {
  ...
  /**
   * Opens the stadium location on Google Maps.
   */
  const handleViewOnGoogleMaps = () => {
    const url = `https://www.google.com/maps/search/?api=1&query=${STADIUM_CENTER.lat},${STADIUM_CENTER.lng}`;
    window.open(url, '_blank', 'noopener,noreferrer');
  };

  return (
    <div className="space-y-6 animate-fade-in" role="main" aria-label="Stadium Live Monitoring">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {/* Status Legend */}
          <div className="flex items-center gap-3 px-4 py-2 glass-panel" aria-label="Status Legend">
            {Object.entries(statusColors).map(([status, color]) => (
              <div key={status} className="flex items-center gap-1.5" title={`${status} status`}>
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          <GlassPanel className="p-0 overflow-hidden h-[600px] relative border-neon-cyan/20">
            ...
            <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between pointer-events-none">
              <div className="glass-panel p-3 pointer-events-auto flex flex-col gap-2">
                <div>
                ...
                <button 
                  ...
                  className="flex items-center gap-2 text-[10px] text-neon-cyan hover:text-white transition-colors py-1 px-2 rounded bg-neon-cyan/10 border border-neon-cyan/20"
                  aria-label="View stadium location on Google Maps"
                >
        ...
        <div className="space-y-4">
          ...
          <GlassPanel className="p-4">
            ...
            <div className="flex flex-wrap gap-2" role="group" aria-label="Zone type filters">
              <button
...
/**
 * StadiumVisualization Component
 * Renders an interactive SVG map with accessible zone nodes.
 */
function StadiumVisualization({
  ...
  return (
    <svg 
      ...
      aria-label="Stadium zone layout"
      role="application"
    >
      <defs>
      ...
      {/* Stadium Structure Overlay */}
      <ellipse cx="50" cy="50" rx="30" ry="22" fill="none" stroke="rgba(0,240,255,0.08)" strokeWidth="0.3" strokeDasharray="2 1" aria-hidden="true" />
      <ellipse cx="50" cy="50" rx="18" ry="12" fill="none" stroke="rgba(34,197,94,0.1)" strokeWidth="0.2" aria-hidden="true" />
      <text x="50" y="50" textAnchor="middle" fill="rgba(34,197,94,0.15)" fontSize="2" fontFamily="monospace" aria-hidden="true">PITCH</text>
      ...
      {zones.map((zone) => {
        ...
        return (
          <g 
            ...
            tabIndex={0}
            role="button"
            aria-label={`${zone.name}, status: ${zone.status}, occupancy: ${Math.round(occupancyRatio * 100)}%`}
          ...
          >
            ...
            {/* Labels */}
            <text x={pos.x} y={pos.y + size + 2} textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="1.6" fontFamily="Inter, sans-serif" aria-hidden="true">
              {zone.name.split(' ').slice(0, 2).join(' ')}
            </text>
            <text x={pos.x} y={pos.y + size + 3.8} textAnchor="middle" fill={color} fontSize="1.3" fontFamily="monospace" fontWeight="bold" aria-hidden="true">
